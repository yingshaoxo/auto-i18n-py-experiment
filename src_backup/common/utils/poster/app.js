let log = console.log; // 如果在項目的APP.vue文件中的onlaunch中設置 console.log = ()=> {} 則在此也不會有打印信息
log = ()=>{};	// 打開注釋則該插件不會打印任何信息
let _app = {
	//交互控制
	log(t) {
		log(t);
	...
	getPosterUrl(objs) { // 後端獲取背景圖的url路徑方法
		let {
			backgroundImage,
			type,
			formData
		...
		return new Promise((rs, rj) => {
			...
			if (backgroundImage) {
				...
				switch (type) { //根據type獲取背景圖, 一般要改成request獲取
					case 1:
						image = '';
						break;
					default:
	...
	shareTypeListSheetArray: {
		array: [0, 1, 2, 3, 4, 5]
	}, // 分享類型 0-圖文鍊接 1-純文字 2-純圖片 3-音樂 4-視頻 5-小程序
	isArray(arg) {
		...
		return new Promise((rs, rj) => {
			...
			if (url.substring(0, 4) === 'http') {
				...
				uni.downloadFile({
					url,
					success(d_res) {
						log('下載背景圖成功：' + JSON.stringify(d_res));
						if (d_res && d_res.tempFilePath) {
							...
							uni.saveFile({
								...
								success(s_res) {
									log('保存背景圖成功:' + JSON.stringify(s_res));
	...
	getImageInfo_PromiseFc(imgPath) {
		return new Promise((rs, rj) => {
			log('准备獲取图片信息:' + imgPath);
			imgPath = checkMPUrl(imgPath);
			uni.getImageInfo({
				src: imgPath,
				success: res => {
					log('獲取图片信息成功:' + JSON.stringify(res));
					rs(res);
				},
				fail: err => {
					log('獲取图片信息失败:' + JSON.stringify(err));
					rj(err)
				}
			})
		});
	...
	readyShare(providerName, WXScene, shareType, title, summary, href, imageUrl, miniProgramObj, mediaUrl, scb, fcb) {
		...
		switch (shareType) {
			...
			case 5:
				...
				break;
			default:
				_this.showToast('分享參數-shareType錯誤');
				return;
				break;
		}
	...
	doingShare(shareObj, shareType, data, scb, fcb) {
		...
		switch (shareType) {
			...
			case 5: //小程序
				if (miniProgramId == '') {
					uni.showToast('未設置小程序ID, 請使用其他方式分享');
					return;
				}
				let mp = {
					id: miniProgramId
				...
				break;
			default:
				appJS.showToast('分享參數-shareType錯誤');
				break;
		}
