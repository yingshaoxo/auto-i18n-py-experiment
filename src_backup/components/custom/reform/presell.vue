          ...
          >
            ...
            <view slot="label">
              <block v-if="items.params.style == 3">
                <view class="label quankuan-label" v-if="item.price_text">{{
                  item.price_text
                }}</view>
              </block>
            </view>
            <view slot="priceRight">
              ...
              <block v-if="items.params.style == 3">
                <view
                  class="label weikuan-label"
                  v-if="item.final_price_text"
                  >{{ item.final_price_text }}</view
                >
              </block>
            </view>
            <view slot="originPrice">
              <block v-if="items.params.style != 3">
                <view
                  class="label weikuan-label"
                  v-if="item.final_price_text"
            ...
            <view slot="sales">
              <block v-if="items.params.style != 3">
                <view class="sales-label" v-if="item.sales_text">{{
                  item.sales_text
                }}</view>
              </block>
            </view>
      ...
      this.getGoodsList()
        .then(list => {
          ...
          list.forEach((e, i) => {
            ...
            if (this.items.params.show_front_price == 1) {
              e.front_price_text = e.firstmoney;
            }
            if (this.items.params.show_final_price == 1) {
              e.final_price_text = `尾款${yuan(e.finalmoney)}`;
            }
            if (this.items.params.show_price == 1) {
              e.price_text = `全款${yuan(e.allmoney)}`;
            }
            if (
              this.items.params.show_presell_sales == 1 &&
              this.items.params.style != 3
            ) {
              e.sales_text = `已定${wan(e.buy_num)}件`;
            }
          });
          this.list = list;
