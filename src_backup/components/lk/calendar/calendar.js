...
var calendar = {
   ...
   * 24節氣速查表
   * @Array Of Property
   * @trans["小寒","大寒","立春","雨水","驚蟄","春分","清明","穀雨","立夏","小滿","芒種","夏至","小暑","大暑","立秋","處暑","白露","秋分","寒露","霜降","立鼕","小雪","大雪","鼕至"]
   * @return Cn string
   */
  solarTerm: [
    "\u5c0f\u5bd2",
   ...
   * 月份转農曆称呼速查表
   * @Array Of Property
   * @trans ['正','一','二','三','四','五','六','七','八','九','十','鼕','腊']
   * @return Cn string
   */
  nStr3: [
    "\u6b63",
  ...
  /**
   * 傳入阳历年月日获得详细的公历、農曆object信息 <=>JSON
   * @param y  solar year
   * @param m  solar month
   * @param d  solar day
   * @return JSON object
  ...
  solar2lunar: function(y, m, d) {
    ...
    for (i = 1; i < 13 && offset > 0; i++) {
      ...
      if (leap > 0 && i == leap + 1 && isLeap == false) {
        --i;
        isLeap = true;
        temp = this.leapDays(year); // 計算農曆閏月天数
      } else {
        temp = this.monthDays(year, i); // 計算農曆普通月天数
      }
      // 解除閏月
  ...
  /**
   * 傳入農曆年月日以及傳入的月份是否閏月获得详细的公历、農曆object信息 <=>JSON
   * @param y  lunar year
   * @param m  lunar month
   * @param d  lunar day
   * @param isLeapMonth  lunar month is leap or not.[如果是農曆閏月第四个參數赋值true即可]
   * @return JSON object
   * @eg:console.log(calendar.lunar2solar(1987,9,10));
   */
  lunar2solar: function(y, m, d, isLeapMonth) {
    // 參數區間1900.1.31~2100.12.1
    ...
    // 转换閏月農曆 需补充该年閏月的前一个月的時差
    if (isLeapMonth) {
      offset += day;
    }
    // 1900年農曆正月一日的公历時间为1900年1月30日0時0分0秒(该時间也是本農曆的最开始起始点)
