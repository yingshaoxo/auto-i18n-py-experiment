/*
  將 html 解析為適用於小程序 rich-text 的 DOM 結構
...
var emoji; // emoji 補丁包 https://jin-yufeng.github.io/Parser/#/instructions?id=emoji
class MpHtmlParser {
		...
		switch (node.name) {
			...
			case 'audio':
				...
				if (!attrs.controls && !attrs.autoplay)
					console.warn(`存在沒有 controls 屬性的 ${node.name} 標籤，可能導致無法播放`, node);
				this.bubble();
				break;
			case 'td':
			case 'th':
