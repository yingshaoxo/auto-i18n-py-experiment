<!--
  ...
  插件市場：https://ext.dcloud.net.cn/plugin?id=805
  author：JinYufeng
  update：2020/04/26
-->
<template>
...
<script>
	...
	var document; // document 補丁包 https://jin-yufeng.github.io/Parser/#/instructions?id=document
	// 計算 cache 的 key
	function hash(str) {
	...
	export default {
		...
		methods: {
			...
			_handleHtml(html, append) {
				...
				if (!append) {
					// 處理 tag-style 龢 userAgentStyles
			...
			setContent(html, append) {
				...
				if (!html)
					...
					nodes = html.nodes;
					console.warn('錯誤的 html 類型：object 類型已废弃');
				} else
					return console.warn('錯誤的 html 類型：' + typeof html);
