 ...
 * uni-app平臺高性能跨全端圖表，支持H5、APP、小程序（微信/支付寶/百度/頭條/QQ/360）
 * Copyright (c) 2019 QIUN秋雲 https://www.ucharts.cn All rights reserved.
...
function calXAxisData(series, opts, config){
    ...
    // 計算X軸刻度的屬性譬如每個刻度的間隔,刻度的起始點\結束點以及總長
		...
		for(let i=0;i<splitPointList[0].length;i++){
			if(i>leftNum && i<rightNum){
				...
				if (data[i][1] - data[i][0] > 0) {
					...
					context.setLineWidth(1 * opts.pixelRatio);
					context.moveTo(item[3].x, item[3].y); //頂點
					context.lineTo(item[1].x, item[1].y); //收盤中间点
					context.lineTo(item[1].x - eachSpacing / 4, item[1].y); //收盤左侧点
					context.lineTo(item[0].x - eachSpacing / 4, item[0].y); //開盤左侧点
					context.lineTo(item[0].x, item[0].y); //開盤中间点
					context.lineTo(item[2].x, item[2].y); //底點
					context.lineTo(item[0].x, item[0].y); //開盤中间点
					context.lineTo(item[0].x + eachSpacing / 4, item[0].y); //開盤右侧点
					context.lineTo(item[1].x + eachSpacing / 4, item[1].y); //收盤右侧点
					context.lineTo(item[1].x, item[1].y); //收盤中间点
					context.moveTo(item[3].x, item[3].y); //頂點
				} else {
...
function drawGaugeDataPoints(categories, series, opts, config, context) {
  ...
  var innerRadius = radius - gaugeOption.width;
	...
	//判斷儀表盤的樣式：default百度樣式，progress新樣式
	if(gaugeOption.type == 'progress'){
		...
		//配置漸變填充（起點：中心點向上減半徑；結束點中心點向下加半徑）
...
if (typeof module === "object" && typeof module.exports === "object") {
  ...
  //export default Charts;//建議使用nodejs的module導出方式，如報錯請使用export方式導出
}
