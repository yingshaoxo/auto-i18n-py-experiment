  ...
  <view class="pages">
    <lk-load-list ref="load" @init="loadInit" :down="downOption" @down="downCallback" :up="upOption" @up="upCallback">
      <lk-cell :value="item.create_time" v-for="(item, index) in list" :key="index"
        ...
        <view slot="title">{{ item.remark }}</view>
        <view slot="label" class="label" :class="item.status == 4 || item.status == -1 ? 'text-through' : ''"> </view>
        <view>
          <view :class="item.money > 0 ? 'positive' : 'negative'">
            {{ item.money }}
          </view>
