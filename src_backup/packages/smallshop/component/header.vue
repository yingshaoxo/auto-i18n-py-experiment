      ...
      <view class="box" :class="boxClasses" v-if="styleType">
        ...
        <view class="info">
          <view class="info-name">
            ...
            <!-- <view class="tag" v-if="url_wx_id == wx_id_from_vuex" @click="goToTheEditPage(url_wx_id)">
              <lk-icon color="inherit" size="10" :name="'edit'" />
              編輯
            </view> -->
          </view>
          <view class="info-foot">
            <view class="fans">
              <text>粉絲</text>
              <text>{{ " " + shopInfo.collection }}</text>
            </view>
            <view class="score">
              <text class="text">綜合評分</text>
              <!-- <lk-star :value="shopInfo.score" :color="starColor" :voidColor="starColor" /> -->
              <lk-star :value="5" :color="starColor" :voidColor="starColor" />
            </view>
            <view class="tag" v-if="styleType == 4" @click="onCollect">
              <lk-icon color="inherit" size="10" :name="collectInfo.icon" />
              {{ collectInfo.text }}
...
export default {
  ...
  computed: {
    ...
    imageBg() {
      ...
      if (this.info && this.info.background) {
  ...
  mounted() {
    ...
    this.collect.isCollect = shopInfo.is_collection == 1 ? true : false;
    this.getConfig().then(() => {
  ...
  methods: {
    onCollect() {
      const wx_id = this.routeInfo.query.wx_id;
    ...
    goToTheEditPage() {
      if (this.url_wx_id == this.wx_id_from_vuex) {
        this.$Navigate.push("/packages/smallshop/shop?wx_id=" + this.url_wx_id);
      }
    },
  },
  components: {},
