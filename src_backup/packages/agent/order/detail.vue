<template>
  <view class="pages">
    <block v-if="buy_type == 'pickupgoods' || buy_type == 'retail'">
      ...
      <detail-logistic :status="detail.order_status" :order_id="detail.order_id" :info="logistic_info" />
    </block>
    <detail-goods :buy_type="buy_type" :detail="detail" />
    <detail-message v-if="buy_type == 'pickupgoods' || buy_type == 'retail'" :detail="detail" />
    <detail-info :buy_type="buy_type" :detail="detail" />
    <detail-log :buy_type="buy_type" :detail="detail" />
    <view v-if="detail.order_status == 0 || detail.order_status == 2" class="footer">
      <foot-operate :list="detail.member_operation" :order_id="order_id" @init="getData" />
    </view>
  </view>
</template>

<script>
import { GET_ORDERDETAIL } from "@/common/interface/agent";
