<template>
  <view class="pages">
    <view class="header">
      <lk-search v-model="params.search_text" placeholder="訂單號/店鋪名稱/商品名稱"
        :action="params.buy_type != 'commission' ? '搜索' : ''" @search="onSearch" />
      <lk-tabs v-model="active" :active-color="theme.color" :line-color="theme.color"
        :nav-per-view="tabs.length > 4 ? 'auto' : tabs.length" slot-title @change="onTabChange">
    ...
    <lk-load-list ref="load" top="196" @init="loadInit" :down="downOption" @down="downCallback" :up="upOption"
      ...
      <view class="list" v-if="params.buy_type == 'commission'">
        <orderCommissionItem v-for="(item, index) in list" :key="index" :item="item" />
      </view>
      <view class="list" v-else>
        <view class="item card-group-box" v-for="(items, index) in list" :key="index">
          ...
          <lk-goods-card v-for="(item, goods_index) in items.goods_list" :key="goods_index" cell-padding lazyLoad
            :desc="item.sku_name" :num="item.num" :price="item.price" :title="item.goods_name" :image="item.goods_img">
          ...
          <lk-cell>
            <foot-operate :buy_type="buy_type" :list="items.operateItems" :order_id="items.order_id"
              @init="resetList" />
          </lk-cell>
        </view>
      </view>
