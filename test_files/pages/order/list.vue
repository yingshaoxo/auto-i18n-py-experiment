    ...
    <lk-load-list ref="load" top="196" @init="loadInit" :down="downOption" @down="downCallback" :up="upOption"
      ...
      <view class="list">
        <view class="item card-group-box" v-for="(items, index) in list" :key="index">
          ...
          <lk-goods-card v-for="(item, goods_index) in items.order_goods" :key="goods_index" cell-padding lazyLoad
            ...
            <view slot="footer">
              <block v-if="item.member_operation && item.member_operation.length">
                <lk-button size="mini" round v-for="(btn, btn_index) in item.member_operation" :key="btn_index"
                  @click="onResult(index, item.order_goods_id)">
                  {{ btn.name }}
                </lk-button>
              </block>
            </view>
          ...
          <lk-cell>
            <foot-operate :info="items" @init="resetList" />
          </lk-cell>
        </view>
      </view>
    </lk-load-list>
  ...
  computed: {
    ...
    tabs () {
      return [
        ...
        {
          name: `待付款(${this.info.unpaidOrder | 0})`,
          status: 0
        },
        {
          name: `待發貨(${this.info.shipmentPendingOrder | 0})`,
          status: 1
        },
        {
          name: `待收貨(${this.info.goodsNotReceivedOrder | 0})`,
          status: 2
        },
        {
          name: `待評價(${this.info.un_evaluate_num | 0})`,
          status: -2
        },
        {
          name: `售後(${this.info.refundOrder | 0})`,
          status: -1
        }
      ]
    }
        ...
        .then(({ data }) => {
          let list = data.order_list.map(e => {
            ...
            e.order_goods.forEach(g => {
              g.to = "/packages/order/detail?order_id=" + e.order_id;
            });
