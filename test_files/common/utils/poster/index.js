...
const ShreUserPosterBackgroundKey = "ShrePosterBackground_"; // 背景圖片緩存名稱前綴
const idKey = "QSSHAREPOSTER_IDKEY"; //drawArray自動生成的idkey
var isMp = false;
// #ifdef MP
isMp = true;
// #endif
...
var nbgScale = 1;
// export default
export default function getPoster(obj) {
...
function returnPromise(obj) {
  ...
  return new Promise(async (rs, rj) => {
    try {
      _app.showLoading("正在準備海報數據");
      if (!Context) {
        _app.log("沒有畫布對象,創建畫布對象");
      ...
      _app.log("獲取背景圖信息對象成功:" + JSON.stringify(bgObj));
      const params = {
        bgObj,
        type,
        bgScale,
      ...
      if (imagesArray) {
        ...
        _app.showLoading("正在生成需繪製圖片的臨時路徑");
        _app.log("準備設置圖片");
      ...
      if (qrCodeArray) {
        ...
        for (let i = 0; i < qrCodeArray.length; i++) {
          ...
          if (qrCodeArray[i].image)
            qrCodeArray[i].image = await _app.downloadFile_PromiseFc(
              qrCodeArray[i].image
            );
        }
        _app.hideLoading();
        ...
        if (_app.isArray(drawArray) && drawArray.length > 0) {
            ...
            switch (drawArrayItem.type) {
              ...
              case "qrcode":
                if (drawArrayItem.image)
                  newData = {
                    image: await _app.downloadFile_PromiseFc(
                      drawArrayItem.image
                    )
                  };
                break;
          ...
          if (hasAllInfoCallback) {
            ...
            drawArray_copy.sort((a, b) => {
              const a_serialNum =
                !_app.isUndef(a.serialNum) && !_app.isNull(a.serialNum)
                  ? Number(a.serialNum)
                  : Number.NEGATIVE_INFINITY;
              const b_serialNum =
                !_app.isUndef(b.serialNum) && !_app.isNull(b.serialNum)
                  ? Number(b.serialNum)
                  : Number.NEGATIVE_INFINITY;
            ...
            for (let i = 0; i < drawArray_copy.length; i++) {
              ...
              if (_app.isFn(item.allInfoCallback)) {
                ...
                if (!_app.isUndef(item_idKey)) {
                  ...
                  console.log("程序錯誤 找不到idKey!!!	...這不應該啊");
                }
              }
            }
            _app.log("for循環结束");
...
function drawShareImage(obj) {
  ...
  return new Promise((rs, rj) => {
    try {
      _app.showLoading("正在繪制海報");
      _app.log("背景對象:" + JSON.stringify(bgObj));
      if (bgObj && bgObj.path) {
        _app.log("背景有圖片路徑");
        ...
        if (bgObj.backgroundColor) {
          _app.log("背景有背景顏色:" + bgObj.backgroundColor);
      ...
      _app.showLoading("繪製可控層級序列");
      if (drawArray && drawArray.length > 0) {
        for (let i = 0; i < drawArray.length; i++) {
          const drawArrayItem = drawArray[i];
          _app.log(
            "繪製可控層級序列, drawArrayItem:" + JSON.stringify(drawArrayItem)
          );
          switch (drawArrayItem.type) {
            case "image":
              _app.log("繪製可控層級序列, 繪製圖片");
              drawImage(Context, drawArrayItem);
              break;
            case "text":
              _app.log("繪製可控層級序列, 繪製文本");
              drawText(Context, drawArrayItem, bgObj);
              break;
            case "qrcode":
              _app.log("繪製可控層級序列, 繪製二維碼");
              drawQrCode(Context, drawArrayItem);
              break;
            case "custom":
              _app.log("繪製可控層級序列, 繪製自定義內容");
              if (
                drawArrayItem.setDraw &&
                typeof drawArrayItem.setDraw === "function"
              )
                drawArrayItem.setDraw(Context);
              break;
              drawRoundStrokeRect, drawStrokeRect;
            case "fillRect":
              _app.log("繪製可控層級序列, 繪製填充直角矩形");
              drawFillRect(Context, drawArrayItem);
              break;
            case "strokeRect":
              _app.log("繪製可控層級序列, 繪製線條直角矩形");
              drawStrokeRect(Context, drawArrayItem);
              break;
            case "roundStrokeRect":
              _app.log("繪製可控層級序列, 繪製線條圓角矩形");
              drawRoundStrokeRect(Context, drawArrayItem);
              break;
            case "roundFillRect":
              _app.log("繪製可控層級序列, 繪製填充圓角矩形");
      ...
      _app.showLoading("繪制中");
      setTimeout(() => {
        _app.log("準備執行draw方法");
        _app.log("Context:" + Context);
        const fn = function() {
          _app.showLoading("正在輸出圖片");
          ...
          const data = {
            ...
            width: bgObj.width,
            height: bgObj.height,
            destWidth: bgObj.width, // 若H5使用這裡請不要乘以二
            destHeight: bgObj.height, // 若H5使用這裡請不要乘以二
            quality: 0.8,
            fileType: "jpg",
            ...setObj
          };
          _app.log("canvasToTempFilePath的data對象:" + JSON.stringify(data));
          canvasToTempFilePathFn = function() {
            const toTempFilePathObj = {
              ...
              fail(err) {
                _app.hideLoading();
                _app.log("輸出圖片失敗:" + JSON.stringify(err));
                rj("輸出圖片失敗:" + JSON.stringify(err));
              }
            };
          ...
          _app.log("延時系數:" + delayTimeScale);
          _app.log("總計延時:" + delayTime);
...
function drawFillRect(Context, drawArrayItem = {}) {
  //填充矩形
  _app.log(
    "进入繪製填充直角矩形方法, drawArrayItem:" + JSON.stringify(drawArrayItem)
  );
  ...
  Context.fillRect(
    drawArrayItem.dx || 0,
    drawArrayItem.dy || 0,
    drawArrayItem.width || 0,
    drawArrayItem.height || 0
  );
...
function setText(Context, texts) {
  // 設置文本數據
  _app.log("進入設置文字方法, texts:" + JSON.stringify(texts));
...
function setTextFn(Context, textItem) {
  _app.log("進入設置文字方法, textItem:" + JSON.stringify(textItem));
  if (_app.isNotNull_string(textItem.text)) {
    ...
    _app.log("字符串信息-初始化默認值後:" + JSON.stringify(textItem));
    const textLength = countTextLength(Context, {
      text: textItem.text,
      size: textItem.size
    });
    _app.log("字符串信息-初始化時的文本長度:" + textLength);
    let infoCallBackObj = {};
    ...
    _app.log("字符串信息-infoCallBack後:" + JSON.stringify(textItem));
  }
  return textItem;
}

function countTextLength(Context, obj) {
  _app.log("計算文字長度, obj:" + JSON.stringify(obj));
  const { text, size } = obj;
  Context.setFontSize(size);
  let textLength;
  try {
    textLength = Context.measureText(text); // 官方文檔說 App端自定義組件編譯模式暫時不可用measureText方法
  } catch (e) {
    //TODO handle the exception
    textLength = {};
  }
  textLength = {};
  _app.log("measureText計算文字長度, textLength:" + JSON.stringify(textLength));
  ...
  return new Promise(async (resolve, rejcet) => {
    try {
      if (images && _app.isArray(images)) {
        _app.log("images是一個數組");
        for (let i = 0; i < images.length; i++) {
          _app.log("設置圖片數據循環中:" + i);
          images[i] = await setImageFn(images[i]);
        }
      } else {
        _app.log("images是一个對象");
  ...
  return new Promise(async (resolve, reject) => {
    try {
      if (image.url) {
        ...
        image.url = imgUrl;
        const hasinfoCallBack =
          image.infoCallBack && typeof image.infoCallBack === "function";
        let imageInfo = {};
...
function drawText(Context, textArray, bgObj) {
  ...
  if (!_app.isArray(textArray)) {
    _app.log("遍歷文本方法, 不是數組");
    textArray = [textArray];
  } else {
    _app.log("遍歷文本方法, 是數組");
  }
  _app.log("遍歷文本方法, textArray:" + JSON.stringify(textArray));
  const newArr = [];
  if (textArray && textArray.length > 0) {
    for (let j = 0; j < textArray.length; j++) {
      ...
      if (textItem.text && textItem.lineFeed) {
        ...
        if (_app.isObject(textItem.lineFeed)) {
          ...
          lineNum =
            lineFeed.lineNum !== undefined &&
            typeof lineFeed.lineNum === "number" &&
            lineFeed.lineNum >= 0
              ? lineFeed.lineNum
              : lineNum;
          maxWidth =
            lineFeed.maxWidth !== undefined &&
            typeof lineFeed.maxWidth === "number"
              ? lineFeed.maxWidth
              : maxWidth;
          lineHeight =
            lineFeed.lineHeight !== undefined &&
            typeof lineFeed.lineHeight === "number"
              ? lineFeed.lineHeight
              : lineHeight;
          dx =
            lineFeed.dx !== undefined && typeof lineFeed.dx === "number"
              ? lineFeed.dx
              : dx;
        }
        ...
        _app.log("循環出的文本数组:" + JSON.stringify(row));
        //只顯示幾行 變量間距lineHeight  變量行數lineNum
        let allNum =
          lineNum >= 0 && lineNum < row.length ? lineNum : row.length;
        ...
        for (let i = 0; i < allNum; i++) {
          ...
          _app.log("重新组成的文本對象:" + JSON.stringify(obj));
          newArr.push(obj);
        }
      } else {
  ...
  _app.log("繪製文本新数组:" + JSON.stringify(newArr));
...
function setFont(textItem = {}) {
  if (textItem.font && typeof textItem.font === "string") {
    ...
    if (
      textItem.fontWeight &&
      (typeof textItem.fontWeight === "string" ||
        typeof textItem.fontWeight === "number")
    )
...
function drawTexts(Context, texts) {
  // 繪製文本
  _app.log("准备繪製文本方法, texts:" + JSON.stringify(texts));
  if (texts && _app.isArray(texts)) {
    _app.log("准备繪製文本方法, 是數組");
    if (texts.length > 0) {
      for (let i = 0; i < texts.length; i++) {
    ...
    _app.log("准备繪製文本方法, 不是數組");
    drawTextFn(Context, texts);
  }
}

function drawTextFn(Context, textItem) {
  _app.log("进入繪製文本方法, textItem:" + JSON.stringify(textItem));
  if (textItem && _app.isObject(textItem) && textItem.text) {
    ...
    if (textItem.lineThrough && _app.isObject(textItem.lineThrough)) {
      ...
      lineThrough.alpha =
        lineThrough.alpha !== undefined ? lineThrough.alpha : textItem.alpha;
      lineThrough.style = lineThrough.style || textItem.color;
      lineThrough.width =
        lineThrough.width !== undefined
          ? lineThrough.width
          : textItem.size / 10;
      lineThrough.cap =
        lineThrough.cap !== undefined ? lineThrough.cap : "butt";
      _app.log("删除线對象:" + JSON.stringify(lineThrough));
...
function drawImage(Context, images) {
  // 繪製圖片
  _app.log("判斷圖片數據類型:" + JSON.stringify(images));
  if (images && _app.isArray(images)) {
    if (images.length > 0) {
      for (let i = 0; i < images.length; i++) {
...
function readyDrawImageFn(Context, img) {
  _app.log("判断繪製圖片形状, img:" + JSON.stringify(img));
  if (img.url) {
    if (img.circleSet) {
...
function drawImageFn(Context, img) {
  ...
  _app.log("繪製默認圖片方法, 繪製完畢");
}

function drawCircleImage(Context, obj) {
  _app.log("進入繪製圓形圖片方法, obj:" + JSON.stringify(obj));
...
function drawRoundRectImage(Context, obj) {
  // 繪製矩形
  _app.log("进入繪製矩形图片方法, obj:" + JSON.stringify(obj));
  Context.save();
  ...
  drawImageFn(Context, obj);
  Context.restore();
  _app.log("进入繪製矩形图片方法, 繪製完畢");
}

// export
function drawQrCode(Context, qrCodeObj) {
  //生成二維碼方法， 參考了 詩小柒 的二維碼生成器代碼
  _app.log("进入繪製二維碼方法");
  _app.showLoading("正在生成二維碼");
  let qrcodeAlgObjCache = [];
  let options = {
    text: String(qrCodeObj.text || "") || "", // 生成內容
    size: Number(qrCodeObj.size || 0) || 200, // 二維碼大小
    background: String(qrCodeObj.background || "") || "#ffffff", // 背景色
    foreground: String(qrCodeObj.foreground || "") || "#000000", // 前景色
    pdground: String(qrCodeObj.pdground || "") || "#000000", // 定位角點顏色
    correctLevel: Number(qrCodeObj.correctLevel || 0) || 1, // 容錯級別
    image: String(qrCodeObj.image || "") || "", // 二維碼圖標
    imageSize: Number(qrCodeObj.imageSize || 0) || 40, // 二維碼圖標大小
    dx: Number(qrCodeObj.dx || 0) || 0, // x軸距離
    dy: Number(qrCodeObj.dy || 0) || 0 // y軸距離
  };
  let qrCodeAlg = null;
  let d = 0;
  ...
  for (let row = 0; row < count; row++) {
    for (let col = 0; col < count; col++) {
      ...
      Context.fillRect(
        options.dx + Math.round(col * tileW),
        options.dy + Math.round(row * tileH),
        w,
        h
      );
    }
  ...
  _app.log("进入繪製二維碼方法完毕");
  _app.hideLoading();
}

function getShreUserPosterBackground(objs) {
  //檢查背景圖是否存在於本地， 若存在直接返回， 否則調用getShreUserPosterBackgroundFc方法
  let { backgroundImage, type } = objs;
  return new Promise(async (resolve, reject) => {
    try {
      _app.showLoading("正在獲取海報背景圖");
      const savedFilePath = await getShreUserPosterBackgroundFc(objs);
      _app.hideLoading();
      resolve(savedFilePath);
    } catch (e) {
      _app.hideLoading();
      _app.showToast("獲取分享用戶背景圖失敗:" + JSON.stringify(e));
...
function getShreUserPosterBackgroundFc(objs, upimage) {
  //下載並保存背景圖方法
  let { backgroundImage, type } = objs;
  _app.log("獲取分享背景圖, 嘗試清空本地數據");
  return new Promise(async (resolve, reject) => {
    try {
      _app.showLoading("正在下載海報背景圖");
      _app.log("没有从後端获取的背景图片路径, 尝试从後端获取背景图片路径");
      let image = backgroundImage
        ? backgroundImage
        : await _app.getPosterUrl(objs);
      image = await base64ToPathFn(image);
      _app.log("嘗試下載並保存背景圖:" + image);
      const savedFilePath = await _app.downLoadAndSaveFile_PromiseFc(image);
      if (savedFilePath) {
        _app.log("下載並保存背景圖成功:" + savedFilePath);
        ...
        _app.log("拼接背景图信息對象成功:" + JSON.stringify(returnObj));
