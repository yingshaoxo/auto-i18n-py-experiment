  ...
  <view class="pages">
    <view>
      <lk-cell :title="paramText.usable" field>
        <view class="price-color first-letter">{{ paramText.number }}</view>
      </lk-cell>
...
export default {
  ...
  computed: {
    ...
    isDisabled() {
      return !parseFloat(this.memberInfo.balance);
    },
    paramText() {
      return {
        ...
        input: this.pageTypeText + this.memberTexts.balance_style,
        inputPlaceholder:
          "請輸入" + this.pageTypeText + this.memberTexts.balance_style,
        remark: "備注",
  ...
  methods: {
    ...
    onInput(e) {
      let money = parseFloat(this.memberInfo.balance);
